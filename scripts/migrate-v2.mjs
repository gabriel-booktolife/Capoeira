...
const firestoreResourceRoot = `projects/${projectId}/databases/${databaseId}/documents`;
const firestoreRoot = `https://firestore.googleapis.com/v1/${firestoreResourceRoot}`;
...
const writes = [];
for (const collection of contentCollections) for (const [id, data] of Object.entries(migrated[collection])) writes.push({ update: { name: `${firestoreResourceRoot}/${collection}/${id}`, fields: encodeFields(data) }, currentDocument: { exists: true } });
writes.push({ update: { name: `${firestoreResourceRoot}/settings/public`, fields: encodeFields(settings) } });
writes.push({ update: { name: `${firestoreResourceRoot}/admins/${owner.localId}`, fields: encodeFields(admin) } });
